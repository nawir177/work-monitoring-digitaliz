    ...
    <div class="relative w-1/3 h-full md:h-auto mt-5">
        ...
        <div class="relative bg-white rounded-xl shadow dark:bg-gray-700 mt-2">
            <button type="button"
                ...
                data-modal-toggle="authentication-modal" id="closeModal">
            ...
            <div class="px-6 py-6 lg:px-8">
                <h1 class="my-6 text-3xl font-inter text-cyan-500 text-center dark:text-white">
                    ADD NEW TASK PROJECT</h1>
                <form class="space-y-6" action="{{ route('admin.task.store') }}" method="post" >
                    @csrf
                    <input type="hidden" name="project_id" value="{{ $project->id }}">
                    <div class="mb-3 flex gap-3 w-full">
                        <div class="w-full">
                            <label for="nama_barang" class="block font-semibold">Task Name</label>
                            <input class="input rounded-md border-slate-300 w-full text-slate-700 bg-slate-100" required type="text"
                                name="task[]  " >
                        </div>
    ...
    function addPeminjaman() {
        var Peminjaman = `<div class="mb-3 flex gap-3 w-full align-content-center">
                                <div class="w-full">
                                    ...
                                    <input class="input rounded-md border-slate-300 w-full text-slate-700 bg-slate-100 input-bordered w-full" required type="text" name="task[]">
                                </div>
                                <div class="mt-6">
                                   <button 
