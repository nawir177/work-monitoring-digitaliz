<x-app-layout>
     <div class="container md:p-2">
          ...
          <div class="flex my-6">
               <a href="{{ route('report_create', $dailyReport->id) }}"
                    class="px-4 py-2 bg-primary font-primaray hover:bg-cyan-400 text-white rounded-lg shadow">Create
               </a>
          </div>
          {{-- content --}}
                    ...
                    <div id="dropdown-project_{{ $value->id }}"
                         ...
                         <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefault">
                              ...
                              @if ($value->user_id == auth()->user()->id)
                                   ...
                                   <li>
                                        <form action="{{ route('daily-report-list.destroy', $value->id) }}"
                                             method="POST">
                                             @csrf
                                             @method('DELETE')
                                             <button type="submit"
